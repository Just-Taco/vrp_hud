...
RegisterNetEvent("vrp_hud2:return")
AddEventHandler("vrp_hud2:return", function (rHunger, rThirst)
    hunger = rHunger
    thirst = rThirst
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        TriggerServerEvent("vrp_hud2:status")
...
Citizen.CreateThread(function()
    ...
    SetMinimapComponentPosition('minimap_mask', 'L', 'B', x + 0.17, y + 0.09,
                                0.072, 0.162)
    SetMinimapComponentPosition('minimap_blur', 'L', 'B', -0.01, -0.03, 0.18,
                                0.22)
    Wait(5000)
